...
class MapGameDisplay < GridGameContainer
    ...
    def build
        ...
        @sub_elements[:whats_arbre] = WhatsArbre.new(@root){@rectangle.relative_to(x: @rectangle.width - 300 - WA_MARGIN, y: WA_MARGIN, height: - 3*WA_MARGIN - WA_BTN_SIZE).assign!(width: WA_WIDTH)}
        super
    end
end