...
class ObjectsMenu < Scrollable
    ...
    class ListObjectElement < UIElement 
        ...
        def build
            ...
            @sub_elements[:name] = Text.new(@root, "Test text", font_size: FONT_SIZE){Rectangle2.new(@rectangle.x, @rectangle.y + text_top_offset, @rectangle.width, @rectangle.height - text_top_offset)}
        end
