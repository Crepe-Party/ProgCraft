...
class EditorContextualMenu < UIElement
    ...
    def build
        ...
        .constrain{@sub_elements[:map_width_label].rectangle.relative_to(y:INPUT_HEIGHT)}
        ...
        .constrain{@sub_elements[:map_width_input].rectangle.relative_to(y: INPUT_HEIGHT + 10)}
        @sub_elements[:map_height_input] = TextInput.new(@root, placeholder: "Enter a height")
        .constrain{@sub_elements[:map_height_label].rectangle.relative_to(y: INPUT_HEIGHT)}
        .add_event(:submit) do |value:, input:|
            val = value.to_i
