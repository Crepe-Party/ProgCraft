...
class CodeDisplay < Scrollable
    ...
    def build
        ...
        @sub_elements[:highlight] =  Rectangle.new(@root, Gosu::Color::rgba(255,255,255,30)).constrain{@scrl_rect.assign(height: LINE_HEIGHT).relative_to!(y: (@line_number || 0)*LINE_HEIGHT)}
        sync
        super
    end
    def vertical?
